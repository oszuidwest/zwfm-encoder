...
type Encoder struct {
	...
	statusCallback   func() // Callback for status updates
}
...
// This should be called before Start() to enable recording status updates.
func (e *Encoder) SetStatusCallback(cb func()) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.statusCallback = cb
}
...
// This should be called after SetStatusCallback and before Start().
func (e *Encoder) InitRecording() error {
	...
	mgr, err := recording.NewManager(apiKey, "", snap.RecordingMaxDurationMinutes, e.statusCallback)
	if err != nil {
